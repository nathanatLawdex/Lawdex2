...
This build adds a DOCX-first upload flow:
- upload the original `.docx` file
- keep the original file downloadable
- extract the DOCX text into the live working copy in-browser
- optionally refine the extracted text before saving

Older `.doc` files can still be stored as originals, but automatic text extraction is only implemented for `.docx`.
